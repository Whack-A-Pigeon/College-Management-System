...
public class SearchPanel extends JPanel implements ActionListener {
		...
		table.addMouseListener(new MouseAdapter()
			{
				public void mousePressed(MouseEvent e)
				{
					...
					if(studentandfacultycombo.getSelectedIndex()==0)
					{
						...
						String coursecode=table.getValueAt(row,0)+"";
	...
	public SearchPanel() {
		...
		String coursename[]=new courseData().getcourseName();
		coursename[0]="All courses";
		coursenamecombo = new JComboBox<String>(coursename);
		
		coursenamecombo.setFont(new Font("Segoe UI", Font.PLAIN, 18));
		coursenamecombo.setBounds(225, 128, 255, 40);
		coursenamecombo.addActionListener(this);
		
		panel.add(coursenamecombo);
	...
	public void createtablemodel()
	{
		...
		if(studentandfacultycombo.getSelectedIndex()==0)
		{
			String defaultquery="select s.coursecode as 'Class' ,s.rollnumber as 'Roll Number',concat(s.firstname,' ',s.lastname) as 'Student Name',c.coursename as 'course Name',concat(c.semoryear,'-',s.semoryear) as 'Sem/Year' from students s left join courses c on s.coursecode=c.coursecode ";
			String query=defaultquery;
			if(coursenamecombo.getSelectedIndex()>0)
			{
				String coursecode=new courseData().getcoursecode(coursenamecombo.getSelectedItem()+"");
				query+=" where s.coursecode='"+coursecode+"'";
		...
		else if(studentandfacultycombo.getSelectedIndex()==1)
		{
			...
			{
				String coursecode=new courseData().getcoursecode(coursenamecombo.getSelectedItem()+"");
				query+=" where f.coursecode='"+coursecode+"'";
				if(semoryearcombo.getSelectedIndex()>0)
				{
