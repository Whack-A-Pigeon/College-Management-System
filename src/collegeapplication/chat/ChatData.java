...
public class ChatData {
	...
	{
		int total=0;
		String course=s.getcourseCode()+" "+new courseData().getsemoryear(s.getcourseCode())+"-"+s.getSemorYear();
		try
		{
			String query="select readby from chat where ((touserid='"+course+" Official Group' or touserid='"+course+" Students Group') and fromuserid!='"+s.getUserId()+"') or touserid='"+s.getUserId()+"'";
			Statement st=con.createStatement();
			ResultSet rs=st.executeQuery(query);
			while(rs.next())
			{
	...
	{
		int total=0;
		String course=f.getcourseCode()+" "+new courseData().getsemoryear(f.getcourseCode())+"-"+f.getSemorYear();
		try
		{
			String query="select readby from chat where ((touserid='"+course+" Official Group' or touserid='"+course+" Faculties Group') and fromuserid!='"+f.getFacultyId()+"') or touserid='"+f.getFacultyId()+"'";
			Statement st=con.createStatement();
			ResultSet rs=st.executeQuery(query);
			while(rs.next())
			{
