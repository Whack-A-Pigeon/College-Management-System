...
public class SubjectPanel extends JPanel implements ActionListener
{
	...
	public SubjectPanel(StudentMain sm)
	{
		...
		this.createtablemodel(sm.s.getcourseCode(),sm.s.getSemorYear() );
	...
	public SubjectPanel(FacultyMain fm)
	{
		...
		scrollPane.setBounds(10, headerlabel.getY()+headerlabel.getHeight()+20, 1096, this.getHeight()-headerlabel.getHeight()-20);
		headerlabel.setHorizontalAlignment(JLabel.LEFT);
		this.createtablemodel(fm.f.getcourseCode(),fm.f.getSemorYear() );
...
//		timer.start();
		...
		selectcourselabel.setHorizontalAlignment(SwingConstants.LEFT);
		selectcourselabel.setForeground(Color.DARK_GRAY);
		selectcourselabel.setFont(new Font("Segoe UI", Font.PLAIN, 20));
		selectcourselabel.setBounds(30, 89, 248, 42);
		...
		coursenamecombo.setBounds(300, 88, 806, 42);
		add(coursenamecombo);
		...
		coursecode=new courseData().getcoursecode();
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(coursenamecombo.getSelectedIndex()==0 ||semoryearcombo.getSelectedIndex()==0)
		{
			scrollPane.setVisible(false);
		}
			if(e.getSource()==coursenamecombo)
			{
			 ...
			 if(e.getSource()==semoryearcombo)
			 {
				if(coursenamecombo.getSelectedIndex()>0 && semoryearcombo.getSelectedIndex()>0)
				{
				 ...
				 int index=coursenamecombo.getSelectedIndex();
				 this.createtablemodel(coursecode[index-1], sem);
			 ...
			 if(e.getSource()==addsubject)
			 {
				 int sem=semoryearcombo.getSelectedIndex();
				 int index=coursenamecombo.getSelectedIndex();
				 AddSubjectDialog sd=new AddSubjectDialog(coursecode[index-1],sem,table);
	...
	public void createtablemodel(String coursecode,int sem)
	{
		 ResultSet st=new SubjectData().getSubjectinfo(coursecode,sem);
